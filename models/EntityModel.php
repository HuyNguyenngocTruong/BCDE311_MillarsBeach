...
abstract class EntityModel
{
	...
	public function save() {
		if ($this->needsSave()) {
			...
			$sql .= " where memberID = ".$this->primaryKey;
	...
	*/
	...
	public function create() {
        $sql = 'INSERT INTO ' . $this->table . ' (' . $this->fieldNames() . ') VALUES (';
        $values = [];
        foreach ($this->fields as $field => $format) {
            $values[] = $this->getValueString($field, $this->data[$field]);
        }
        $sql .= implode(', ', $values) . ')';
	...
	public function delete() {
        $sql = 'DELETE FROM ' . $this->table . ' WHERE ' . $this->primaryKey . ' = ' . $this->getValueString($this->primaryKey, $this->ID);
        $this->db->execute($sql);
    }
}
?>
